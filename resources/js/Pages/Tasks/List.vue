		...
		<div class="py-12">
			<div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
				<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-16">
					<fieldset v-if="tasks.length > 0" class="space-y-3">
						<legend class="text-lg font-medium text-gray-900">{{ numTasksRemaining }} / {{ numTasks }} tasks remaining</legend>
						<TaskItem v-for="(task, index) in tasks" :key="task.id" :task="task" class="relative flex items-start" @delete-task="deleteTask(task, index)" />
					</fieldset>
			...
			createTask () {
				...
				axios.post('/tasks', { name })
					.then(response => {
						...
						this.tasks[tempIndex] = response.data
					})
					.catch(error => {
